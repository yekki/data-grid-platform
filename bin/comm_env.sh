...
DGP_NODE_TYPE=nodes
DGP_RUNNING_NODES=(node01 node02 management)
DGP_VERBOSE=false
GDP_DEBUG=false
...
function status_node {
	...
	do
        ...
        #if [[ $2 = true ]] || [[ "$2" = "" ]]; then echo "${DGP_DATA_PROCESS_NAME} pid=${pid} for ${DGP_NODE_NAME} is running."; fi
	done
	...
	then
		...
		if [[ $2 = true ]] || [[ "$2" = "" ]]; then echo "${_PROCESS_COUNT} ${DGP_DATA_PROCESS_NAME} process${_B2} for ${DGP_NODE_NAME} $_B1 running."; fi

		return $_PROCESS_COUNT
	else
		if [[ $2 = true ]] || [[ "$2" = "" ]]; then echo "0 ${DGP_DATA_PROCESS_NAME} process for ${DGP_NODE_NAME} is running"; fi
